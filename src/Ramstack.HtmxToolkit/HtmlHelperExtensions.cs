...
{
    private static readonly HtmlString _script = new($"<script>{HtmxAssets.Script}</script>");
    private static readonly HtmlString _debugScript = new($"<script>{HtmxAssets.DebugScript}</script>");
    ...
    /// <summary>
    /// Returns an HTML string of the script tag containing the javascript for htmx
    /// to integrate with the anti-forgery feature of ASP.NET Core.
    /// </summary>
